...
vertex ParticleVertexOut particleVertex(const device float4 *particles [[buffer(0)]],
    ...
    float centerMotionGate = smoothstep(0.18, 0.46, lengthP);
    float quietInterior = interior * centerMotionGate;
    ...
    float tuneRotationSpeed = saturate(uniforms.rotationSpeed);
    ...
    float innerFlowStrength = quietInterior * (0.0012 + 0.0018 * seedB) * uniforms.coreStability;
    ...
    float radialDrift = localBreath * centerMotionGate * (0.42 + edge * 0.72)
        + globalReference * centerMotionGate
        + edgeMembrane;
    ...
    float innerMorph = (quietInterior * 0.20 + midBand * 0.88 * stateFocus) * (0.0100 + 0.0170 * seedB)
    ...
    float2 directionWarp = coherentDirectionField(p, lengthP, depth, fieldTime, edge, quietInterior, midBand, globalAxis, globalSide, globalWave);
    float2 localWarp = localNoiseField(p, depth, fieldTime, particleSeed, seedB, edge, quietInterior, midBand, globalAxis, globalSide, globalWave);
    ...
    p += directionWarp * (0.34 + surfaceMotion * 0.66) * centerMotionGate * mix(1.0, 1.10, thinking * (quietInterior + midBand)) * mix(1.0, 0.88, loading) * tuneFlowStrength;
    p += localWarp * (0.20 + surfaceMotion * 0.80) * centerMotionGate * mix(1.0, 0.58, thinking) * mix(1.0, 0.68, loading) * tuneFlowStrength;
    ...
    float rotationTime = t * (0.20 + tuneRotationSpeed * 5.40);
    float rotationPhaseTime = rotationTime + tuneRotationPhase;
    float directionPulse = sin(rotationTime * 0.72 + tuneRotationPhase);
    float directionRoll = cos(rotationTime * 0.43 + tuneRotationPhase);
    float selfSpinAngle = t * tuneRotationSpeed * 5.40 + tuneRotationPhase;
    float3 selfSpinAngles = float3(
        -tuneRotationDirection.y * selfSpinAngle,
        tuneRotationDirection.x * selfSpinAngle,
        0.0
    );
    ...
    float bodyDepth = modelDepth + globalWave * (0.018 * centerMotionGate + midBand * 0.052 + edge * 0.050) + centerFollow * 0.012 * centerMotionGate;
    float3 body = float3(p.x, p.y, bodyDepth);
    float activeInterior = quietInterior;
    ...
    float coreFlowGate = saturate(centerMotionGate + edge * 0.50 + midBand);
    materialFlow *= tuneFlowStrength * coreFlowGate;
    cloudFlow *= tuneFlowStrength * coreFlowGate;
    ...
    wholeTurn += (tuneRotationDirection.x - tuneRotationDirection.y) * sin(rotationTime * 0.34 + 0.8) * 0.045 * tuneRotationEmphasis;
    ...
    float postSurfaceMotion = centerMotionGate * (0.42 + 0.58 * smoothstep(0.10, 0.58, length(p)));
    ...
    p += surfaceFlowAxis * turnWake * (0.004 + quietInterior * 0.006 + midBand * 0.014 + frontSheetGate * 0.012 + frontSpreadGate * 0.016 + edge * 0.001) * turnWakeGate * mix(1.0, 1.10, thinking) * mix(1.0, 1.10, speaking);
    p += surfaceFlowSide * turnWakeB * (0.003 + quietInterior * 0.004 + midBand * 0.010 + frontSheetGate * 0.008 + frontSpreadGate * 0.012 + edge * 0.001) * turnWakeGate * mix(1.0, 1.06, thinking) * mix(1.0, 1.08, speaking);
    ...
    p += (turnedAxis * sin(fieldTime * 0.33 + 0.4) * 0.018
        + turnedSide * cos(fieldTime * 0.29 + 1.2) * 0.012) * centerMotionGate * (0.34 + postSurfaceMotion * 0.66);
    float visibleWakeGate = turnWakeGate
        * (0.46 + midBand * 0.42 + quietInterior * 0.36 + frontSheetGate * 0.42 + frontSpreadGate * 0.88)
        * (1.0 - smoothstep(0.76, 0.98, stableRadius));
    p += (surfaceFlowAxis * turnWake + surfaceFlowSide * turnWakeB * 0.72)
        * (0.004 + quietInterior * 0.010 + midBand * 0.014 + frontSheetGate * 0.014 + frontSpreadGate * 0.022 + edge * 0.001)
        * visibleWakeGate
        * mix(1.0, 1.20, speaking);
    float focusGate = thinking
        * frontSurfaceGate
        * centerMotionGate
        * (0.28 + quietInterior * 0.34 + midBand * 0.74)
    ...
    p += surfaceFlowAxis * focusWave * (0.006 + midBand * 0.018 + quietInterior * 0.010) * focusGate;
    p -= safeNormalize2(p, radial) * (0.008 + midBand * 0.013 + quietInterior * 0.006) * focusGate;
    float loadingLoopGate = loading
        * centerMotionGate
        * (0.30 + quietInterior * 0.56 + midBand * 0.66 + frontSheetGate * 0.22)
    ...
    p += surfaceFlowSide
        * loadingRing
        * (0.006 + quietInterior * 0.013 + midBand * 0.025 + frontSheetGate * 0.011)
        * loadingLoopGate;
    p += surfaceFlowAxis
        * loadingLayer
        * (0.004 + quietInterior * 0.009 + midBand * 0.019 + frontSpreadGate * 0.008)
        * loadingLoopGate;
    p += safeNormalize2(p, radial)
        * (loadingCycle - 0.5)
        * (0.003 + midBand * 0.009 + quietInterior * 0.007)
        * loadingLoopGate;
    float errorDisruptionGate = error
        * frontSurfaceGate
        * centerMotionGate
        * (0.24 + quietInterior * 0.36 + midBand * 0.86 + frontSheetGate * 0.36)
    ...
    p += surfaceFlowSide
        * errorShear
        * (0.009 + quietInterior * 0.014 + midBand * 0.038 + frontSheetGate * 0.019)
        * errorDisruptionGate;
    p -= surfaceFlowAxis
        * errorStall
        * (0.007 + quietInterior * 0.010 + midBand * 0.026)
        * errorDisruptionGate;
    p += safeNormalize2(p, radial)
        * (errorFracture - 0.5)
        * (0.007 + quietInterior * 0.008 + midBand * 0.022)
        * errorDisruptionGate;
    p += (surfaceFlowSide * errorJitter + surfaceFlowAxis * errorJitterB * 0.54)
        * (0.006 + quietInterior * 0.008 + midBand * 0.024 + frontSheetGate * 0.012)
        * errorJitterGate;
    ...
    float speakingFlowGate = speaking
        * speakingSurfaceGate
        * centerMotionGate
        * (0.24 + quietInterior * 0.42 + midBand * 0.58 + edge * 0.28)
        * (1.0 - smoothstep(0.84, 1.04, stableRadius));
    float speakingPulseSigned = speakingPulse - 0.42;
    p += safeNormalize2(p, radial)
        * speakingPulseSigned
        * (0.008 + quietInterior * 0.016 + midBand * 0.022 + edge * 0.014)
        * speakingFlowGate;
    p += surfaceFlowAxis
        * speakingPulseSigned
        * (0.006 + quietInterior * 0.008 + midBand * 0.016 + frontSpreadGate * 0.016)
        * speakingFlowGate;
    ...
    p *= tuneGlobalScale * max(0.20, uniforms.bodyTransform.z);
    ...
    float cloudDensity = saturate((cloudPatchA * 0.58 + cloudPatchB * 0.42)
        * (0.44 + quietInterior * 0.56 + midBand * 0.46 + edge * 0.12)
    ...
    float sparseCavity = smoothstep(0.62, 0.94, (1.0 - sectionC) * 0.46 + (1.0 - cloudDensity) * 0.38)
        * (quietInterior * 0.20 + midBand * 0.16);
    ...
    float spineSurfaceGate = frontIonGate * (0.24 + quietInterior * 0.30 + edge * 0.46);
    ...
    float layerDensity = saturate(dynamicDensity * (0.18 + quietInterior * 0.30 + midBand * 0.50 + edge * 0.08)
        + ridgeFlow * 0.10
        + ionCluster * 0.22
        + cloudDensity * 0.22
        + denseSection * 0.22
    ...
    float axisSurfacePresence = smoothstep(0.42, 0.04, stableScreenRadius)
        * (0.18 + 0.82 * smoothstep(0.18, 0.76, abs(visibleDepth)));
    ...
    visibleLocalRidge = mix(visibleLocalRidge, calmLocalRidge, coreCalm * 0.88);
    visibleDenseSection = mix(visibleDenseSection, saturate(visibleCloudDensity * 0.50 + bodyEnvelope * 0.30), coreCalm * 0.72);
    float innerDustPresence = coreCalm * (0.08 + axisSurfacePresence * 0.34 + visibleCloudDensity * 0.08);
    visibleLayerDensity = saturate(visibleLayerDensity + innerDustPresence * 0.11);
    visibleDenseSection = saturate(visibleDenseSection + innerDustPresence * 0.035);
    visibleLocalRidge = saturate(visibleLocalRidge + axisSurfacePresence * 0.012 + coreCalm * visibleCloudDensity * 0.008);
    visibleSparseCavity *= 1.0 - coreCalm * 0.86;
    visibleIonCluster *= 1.0 - coreCalm * 0.82;
    visibleStructuralSpine *= 1.0 - coreCalm * 0.78;
    visibleRidgeFlow *= 1.0 - coreCalm * 0.72;
    float thinkingRidgeGate = thinking * frontDepthGate * (0.30 + midBand * 0.70 + quietInterior * 0.22);
    ...
    float loadingRidgeGate = loading
        * frontDepthGate
        * (0.24 + midBand * 0.72 + quietInterior * 0.42 + edge * 0.04)
    ...
    float speakingRidgeGate = speaking
        * frontDepthGate
        * (0.24 + midBand * 0.54 + quietInterior * 0.28 + edge * 0.18)
    ...
    float errorBreakGate = error
        * frontDepthGate
        * (0.30 + midBand * 0.82 + quietInterior * 0.24 + edge * 0.30)
    ...
    layeredPointSize = mix(layeredPointSize, layeredPointSize * 0.72 + 0.98, coreCalm * 0.44);
...
fragment half4 particleFragment(ParticleVertexOut in [[stage_in]],
    ...
    float ionRidge = litFront * saturate(ridge * 2.10 + densityLight * 0.14 + in.flow * 0.58);
    float ridgeGlow = saturate(ridge * 3.92 + in.flow * 0.94 + densityLight * 0.12)
