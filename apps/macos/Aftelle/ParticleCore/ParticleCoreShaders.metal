...
vertex ParticleVertexOut particleVertex(const device float4 *particles [[buffer(0)]],
    ...
    float edgeScatterCoverage = edgeCoverageBase * mix(0.94, 1.08, edgeVariation);
    ...
    float edgeScatterSpread = mix(0.04, 0.26, edgeScatterReach);
    float2 randomScatterDirection = normalize(screenNormal * (0.84 + 0.26 * seedB) + screenTangent * sin(phaseB + particleSeed * 6.2831853) * edgeScatterSpread);
    float2 surfaceScatterDirection = normalize(randomScatterDirection + screenTangent * surfaceScatterWobble * edgeScatterSpread);
    float edgeScatterTail = 0.72 + seedB * (0.46 + edgeScatterReach * 1.12);
