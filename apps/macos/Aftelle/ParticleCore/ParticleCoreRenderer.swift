...
final class ParticleCoreRenderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        if !didLogDraw {
            ...
            print("[ParticleCore] draw called drawableSize=\(Int(drawableSize.width))x\(Int(drawableSize.height)) particleCount=\(model.particles.count) ndcMin=(\(bounds.minX),\(bounds.minY)) ndcMax=(\(bounds.maxX),\(bounds.maxY)) clearColor=(0.035,0.04,0.05,1) globalBreathingRef=\(breathing) edgeBreathingRef=\(edgeBreathing) coreStability=\(coreStability) motion=fast_2d_blob_deformation particleColor=(0.72...0.98,alpha=0.32...0.88)")
        }
