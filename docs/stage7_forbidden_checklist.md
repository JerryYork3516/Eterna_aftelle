...
PASS:仅做文档级 `avatar_mode` 预留,且不改代码 / DR schema / Runtime API / Provider Profile。
REWORK:开始实现 `avatar_mode` 代码、渲染接口或 Debug Panel 展示,但未说明 7.3 范围与边界。
FAIL:UI 绕过 Controller / Orchestration / RuntimeCore 公共入口,把渲染层逻辑写进 RuntimeCore,或把写实 Avatar / AR / 3D 数字人 / 精准 lip sync / Avatar 编辑器推进 Stage 7.3。
